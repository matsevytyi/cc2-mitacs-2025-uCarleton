...
from CybORG.Agents.Wrappers.DeepSetsPermInvEncoder import DeepSetsPermInvEncoder
...
class DeepSetsPermInvWrapper(Env,BaseWrapper):
    def __init__(self, agent_name: str, raw_cyborg, agent=None,
        ...
        self.encoder = DeepSetsPermInvEncoder(
    ...
    def reset(self, **kwargs):
        ...
        if self.action_history:
            ...
            csv_path = os.path.join(csv_dir, f"actions_{self.agent_name}_DQN_DeepSets_tuning_extended_dynamic_topology.csv")
            ...
            with open(csv_path, mode='a', newline='') as csvfile:
                ...
                for step, action in enumerate(self.action_history):
                    writer.writerow([timestamp, getattr(self, 'episode', None), step, action, len(self.host_order)])
    ...
    def _reload_environment(self, verbose=False):
        ...
        # 1. Grab the full state dictionary of the current DeepSets encoder
        old_state_dict = self.encoder.state_dict()
        
        # 2. Reinitialize the DeepSets encoder 
        # (It naturally handles the new host_order size during forward passes)
        self.encoder = DeepSetsPermInvEncoder(
            observation_space=self.observation_space,
            embedding_dim=64
        ).to(self.device)

        # 3. Load the weights directly back in
        self.encoder.load_state_dict(old_state_dict)
